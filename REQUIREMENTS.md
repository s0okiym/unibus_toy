...
本项目希望用 **纯软件** 的方式，做一个"麻雀虽小五脏俱全"的 UB-like 协议实现，作为学习与验证用的 Toy，重点不在性能而在 **完整地走通从地址翻译到端到端语义** 的整条路径。
...
3. **无连接的 Jetty 抽象**：模仿 UB Jetty 的设计，提供 JFS（发送队列）/ JFR（接收队列）/ JFC（完成队列），不需要预先建立 QP 连接。
4. **拓扑发现与路由**：节点上线后能自动加入超节点、上报本地资源、与其他节点建立可达；支持基于源路由的简单转发。
5. **可靠传输**：在 UDP / TCP 之上自实现轻量重传、序号、ACK，模拟 UB 的链路-传输层可靠性。
6. **基本的流控与拥塞处理**：信用（credit）流控 + 简单的速率反压。
7. **故障检测**：心跳 + 链路故障感知，秒级（toy 级别）通知上层。
8. **CLI / SDK**：提供命令行工具和一个最小的客户端 SDK，能跑通 demo（远端 alloc、写值、读回、原子加、双边发消息、跨节点广播）。
9. **可观测**：基本的日志、计数器、tracing 钩子。

### 2.2 Out-of-Scope（不做）

- 真实硬件协议帧、SerDes、PHY、物理层时序。
- 替代真实 RDMA / NVLink，不追求纳秒级延迟与 TB/s 带宽。
- 完整的多租户安全隔离、加密、权限模型（仅做最简版本）。
- 真实的 nD-FullMesh / Clos 拓扑，仅支持星形 + 全连接两种简化拓扑。
- 与 Linux 内核态、DPDK、io_uring 等深度集成（保留扩展点即可）。
...
- **FR-ADDR-4**：地址翻译表（UB Addr → 本地虚拟地址）由本地节点维护；远端不缓存翻译，只做路由（按 NodeID 转发）。
...
- **FR-MSG-3**：`ub_send_with_imm(..., imm: u64)` —— 在 CQE 中携带 8 字节立即数，避免再读一次内存。
- **FR-MSG-4**：`ub_write_with_imm(..., imm: u64)` —— 远端写完后在对方 JFC 上额外生成一条带立即数的完成事件。
- **FR-MSG-5**：消息有序性：同一对 (src_jetty, dst_jetty) 上的消息按提交顺序到达。
...
- **FR-JETTY-3**：JFS 与 JFC 的槽位一一对应，应用未消费完成事件时硬件（这里是软件 worker）应停止从 JFS 取新 WR，形成自然背压。
...
- **FR-OBS-3**：（可选）暴露一个 HTTP /metrics 端点，便于后续接 Prometheus。
...
| 类别 | 指标 | 说明 |
|---|---|---|
| **规模** | 单 SuperPod 支持 ≥ 16 节点（目标 64） | 不追求更大 |
...
| 阶段 | 内容 |
|---|---|
| **M0** | 需求 + 详细设计 review 通过（本阶段） |
| **M1** | 控制面 + 拓扑发现 + 节点注册：能起 N 个节点组成 SuperPod，CLI 看得到拓扑 |
| **M2** | MR 注册 + 内存语义：跨节点 read/write/atomic 跑通 |
| **M3** | Jetty + 消息语义：send/recv、write_with_imm 跑通 |
| **M4** | 可靠传输 + 流控 + 故障检测：丢包注入测试通过 |
| **M5** | CLI 完善 + 可观测 + demo / bench 脚本 |
