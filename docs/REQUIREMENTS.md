...
1. **Scale-Up 域内的统一编址**：在一个由若干"节点进程"组成的逻辑超节点（SuperPod）里，所有可被访问的资源（CPU 内存段、**NPU 模拟显存**、未来扩展的 SSD 等）拥有一个全局唯一的 UB 地址；任意节点可以根据 UB 地址直接访问对端资源。首版验证"CPU 内存 + 模拟 NPU 显存"两类 Device 共存的统一编址与访问。
2. **两套语义**：
...
10. **Managed 层（M7）**：在 verbs 之上叠一个"节点无感"的全局虚拟地址层，提供 `ub_alloc` / `ub_free` / GVA 访问接口，应用不再显式持有 NodeID，placement / cache / 失效由 UB 负责（详见 Part II 与 FR-GVA）。
...
- **US-9（Managed 层）**：作为 KV cache 应用开发者，我无需关心 NodeID / DeviceID，就能调用 `ub_alloc` 得到一个全局虚拟地址；在任意节点用同一 GVA 读写都能命中 UB 自动选择的"home" 或本地缓存，写入后 UB 负责刷新 / 失效，性能由 UB 根据设备画像尽量优化。
...
- **FR-MR-1**：提供 `ub_mr_register(device, buf_or_offset, len, perms)` → 返回 UB 地址 + MR Handle。`device` 指定 backing 存储：CPU 内存传一个本地指针 / slice；NPU 显存传 `(npu_handle, offset)`。
- **FR-MR-2**：权限位至少包含 `READ / WRITE / ATOMIC`。
- **FR-MR-3**：提供 `ub_mr_deregister(handle)`。
- **FR-MR-4**：MR 的元数据（地址、长度、权限、**device type**）通过控制面广播给 SuperPod 内其他节点（或在按需查询时拉取，二选一，设计阶段决定）。
- **FR-MR-5**：CPU 内存 MR 与 NPU 显存 MR 对远端**完全透明**——远端只看到 UB 地址，发起 `ub_read` / `ub_write` / `ub_atomic_*` 时不区分 device 类型，接收端根据本地 MR Handle 分派到对应 Device 后端执行。
...
| `UB_ERR_FLUSHED` | WR 被冲刷 | Jetty 销毁或 peer 失联时，未完成 WR 批量返回此状态（FR-JETTY-5） |
...
- **FR-OBS-1**：每个节点暴露计数器：`tx_pkts / rx_pkts / retrans / drops / cqe_ok / cqe_err / mr_count / jetty_count`；`mr_count` 按 device type 再分 `mr_count{device="memory|npu"}`。
...
### 5.13 Managed 全局虚拟地址层（FR-GVA，M7 轨）

- **FR-GVA-1**：在 verbs 层之上提供可选的 managed 层 API：`ub_alloc(size, class_hint)` / `ub_free(gva)` / `ub_read_va` / `ub_write_va`。使用 managed 层的应用**不暴露 NodeID / DeviceID / UB Address**；应用只能持有 GVA，由 UB 决定 placement 与路由。
- **FR-GVA-2**：Device / MR 注册需携带静态 capability profile（至少包含带宽、时延、容量、tier 标签）；控制面可查询这些画像，placer 依此做选址；未提供时使用默认 profile。
- **FR-GVA-3**：每个 GVA region 有一个"home"（node + device），由 placer 依据调用方位置与 device 画像选择；跨节点读写同一 GVA 时映射一致。首版允许"home 固定不迁移"，迁移留扩展点。
- **FR-GVA-4**：缓存与一致性按阶段推进：M7.2 = 无缓存，读写直达 home；M7.3 = 可选本地 read cache + FETCH（无 invalidate，reader 用 epoch 自判过期）；M7.4 = 写入需获得写锁并向其他 cache 发送 invalidate（SWMR）。缓存容量管理与逐出策略由 UB 负责，对应用透明。
- **FR-GVA-5**：managed 层生命周期由 UB 统一管理：应用只能通过 `ub_alloc` / `ub_free` 获取 / 释放 region；UB 可基于 MR 画像与进程拓扑做亲和分配，并可在内部做缓存淘汰，而不暴露底层 NodeID。
- **FR-GVA-6**：与 verbs 层互斥使用：一段应用要么走 verbs（显式 NodeID / UB 地址），要么走 managed 层（GVA）；混用留到 M7 后续评估。FR-GVA 不修改既有 FR-MR/FR-MEM/FR-MSG 语义，仅在其之上提供"节点无感"视图。

### 5.14 设备抽象与 NPU 模拟（FR-DEV）

- **FR-DEV-1**：节点内所有可被远端访问的存储均抽象为 **Device**，带 `type` 字段；首版实现 `type ∈ {MEMORY, NPU}`，扩展点留给未来 `SSD` 等。
- **FR-DEV-2**：**NPU 是纯软件模拟**——底层是一段进程内 Rust 分配的字节数组（大小由 `npu.mem_size_mib` 配置，默认 256 MiB），**不涉及**真实 GPU / CUDA / ROCm，也**不模拟算子**（matmul / attention 等全部留空）。本需求只验证 UB 统一编址 + 数据通路能跨 Device 类型工作。
- **FR-DEV-3**：NPU 显存通过同一个 `ub_mr_register` 接口注册为 MR（见 FR-MR-1 新签名），得到的 UB 地址与 CPU 内存 MR 一样可被任意节点 `ub_read` / `ub_write` / `ub_atomic_*` / 作为 `ub_send` 的 payload 源。
- **FR-DEV-4**：UB 地址 `DeviceID` 字段承载 device 身份：`DeviceID=0` 为节点默认 CPU memory device；`DeviceID≥1` 为 NPU 或后续扩展设备；同一节点可同时注册 CPU 与 NPU MR，互不干扰。
- **FR-DEV-5**：首版提供最小的 NPU 本地 API：`ub_npu_open(size) → npu_handle`（启动时调用一次即可），`ub_npu_alloc(npu_handle, len, align) → (offset, len)`（返回相对显存基址的 offset，供 `ub_mr_register` 使用）；无上下文、无 stream、无 copy engine 抽象。
- **FR-DEV-6**：`unibusctl mr list` 必须区分展示 device type，例如：`MR[0] Device=CPU UBAddr=0x...` / `MR[1] Device=NPU#0 UBAddr=0x...`。
- **FR-DEV-7（非目标）**：首版 **不**引入 NPU 之间的 P2P DMA 捷径、**不**做显存 over-commit、**不**模拟 HBM 带宽/延迟差异，这些都留到后续里程碑。
...
| **M2** | Device 抽象（CPU + 模拟 NPU）+ MR 注册 + 内存语义 | 模拟 NPU 模块可启动并分配显存；CPU / NPU 两类 MR 均可注册；跨节点 read/write 对两类 MR 行为一致；跨节点 atomic_cas 并发正确性测试通过；`unibusctl mr list` 显示 device type |
...
| **M7**（可选，独立轨） | Managed 层：GVA + 选址 + Cache + invalidate | `ub_alloc` / `ub_free` 返回 GVA、应用无 NodeID；M7.2 读写直达 home 可用，M7.4 完成 SWMR + invalidate，M7.6 3 节点 KV cache demo 跑通 |
...
| # | 问题 | 默认决策 | 理由 |
|---|---|---|---|
| Q1 | 目标语言 | **Rust**（tokio 运行时） | 既然目标是"逼近用户态软件上限"（§2.2），Rust 的 zero-cost abstraction、内存安全、更严格的类型系统最适合写这类协议栈；tokio 生态（`tokio::net`、`tokio::sync::mpsc`、`tokio::time`）同样适合做 reactor / 队列 / 重传定时器；编译期消除一类并发 bug，对"至多一次"「引用计数 deregister」等语义更友好。 |
| Q2 | 跨语言 SDK | **首版只做 Rust 原生 crate API** | 不浪费精力做 FFI；需要 C ABI 时用 `cbindgen` 生成，留作扩展点。 |
...
| Q5 | 可观测 | **首版就上 HTTP `/metrics`**（Prometheus 文本格式） | Rust 侧用 `metrics` + `metrics-exporter-prometheus` 或 `prometheus` crate，成本低，调试和演示都用得上。 |
| Q6 | 线程模型 | **每节点 = 1 个 reactor tokio task + N 个 worker tokio task**，N 默认 = CPU 数 | 由 tokio 多线程 runtime 调度：单 reactor 处理 socket I/O + 定时器；worker 处理 verb / CQE 派发。语义上仍是"逻辑单 reactor"，方便推理。 |
...
| Q8 | 是否模拟 NPU | **首版做**（纯软件模拟，见 FR-DEV） | 不模拟 NPU 就只剩"同一套地址空间管 CPU 内存"，丢掉了 UB 最核心的卖点；纯内存模拟的 NPU 本质就是一段 Rust `Vec<u8>`，实现成本低，但能让 demo 真正体现"同一个 UB 地址可指向 CPU 或 NPU 存储、同一套 verbs 访问"。不做算子，只做存储层。 |

### 9.1 由上述决策衍生的几个具体约定

- **代码组织**（待详细设计阶段细化）：Cargo workspace，成员大致为 `ub-core`（addr / mr / jetty / verbs）/ `ub-wire` / `ub-transport` / `ub-fabric` / `ub-control` / `ub-obs` / `unibusd`（bin）/ `unibusctl`（bin）。
- **配置格式**：YAML（`serde` + `serde_yaml`）。
- **底层 fabric 默认 UDP**，包大小 ≤ 1400 字节避免 IP 分片，超过部分协议层自己分片。
- **测试**：单元 / 集成测试用 `cargo test`；bench 用 `criterion`；端到端测试用一个脚本起多个 `unibusd` 进程跑 demo。
